...
import lt.codeacademy.anetheticChart.chart.dto.FullChartDTO;
import lt.codeacademy.anetheticChart.chart.helper.MessageService;
...
public class ChartController implements ChartEndPoints {
    ...
    @GetMapping(ChartEndPoints.CHART_ROOT_PATH)
    ...
    @PostMapping(ChartEndPoints.CHART_ROOT_PATH)
    ...
    @GetMapping(ChartEndPoints.PAGE_ROOT_PATH)
    ...
    @GetMapping(ChartEndPoints.UPDATE_ROOT_PATH)
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping(ChartEndPoints.UPDATE_ROOT_PATH)
    public String updateChart(Model model, FullChartDTO fullChartDTO){
        chartService.updateChart(fullChartDTO);
        return "redirect:" + ChartEndPoints.PAGE_ROOT_PATH;
    }

    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping(ChartEndPoints.DELETE_ROOT_PATH)
